...
<div class="almanac-container">
  <div class="almanac-title">程序员老黄历</div>
  <div class="almanac-date"></div>
  <div class="almanac-good">
    <div class="almanac-title">
      <table>
        <tr>
          <td>宜</td>
        </tr>
    ...
    <div class="almanac-clear"></div>
  </div>
  <div class="almanac-split"></div>
  ...
  <div class="almanac-split"></div>
  <div class="almanac-line-tip">
    <strong>座位朝向：</strong>面向<span class="almanac-direction_value"></span>写程序，BUG
    最少。
  </div>
  <div class="almanac-line-tip">
    <strong>今日宜饮：</strong><span class="almanac-drink_value"></span>
  </div>
  <div class="almanac-line-tip">
    <strong>女神亲近指数：</strong><span class="almanac-goddes_value"></span>
  </div>
</div>
<script type="text/javascript">
  (function() {
  	...
  	function addToGood(event) {
  		$('.almanac-good .almanac-content ul').append('<li><div class="almanac-name">' + event.name + '</div><div class="almanac-description">' + event.good + '</div></li>');
  	}

  	// 添加到“不宜”
  	function addToBad(event) {
  		$('.almanac-bad .almanac-content ul').append('<li><div class="almanac-name">' + event.name + '</div><div class="almanac-description">' + event.bad + '</div></li>');
  	}

  	$(function(){
  		if (is_someday()) {document.body.className = 'someday'};
  		$('.almanac-date').html(getTodayString());
  		$('.almanac-direction_value').html(directions[random(iday, 2) % directions.length]);
  		$('.almanac-drink_value').html(pickRandom(drinks,2).join('，'));
  		$('.almanac-goddes_value').html(star(random(iday, 6) % 5 + 1));
  		pickTodaysLuck();
  	});
  })()
</script>
